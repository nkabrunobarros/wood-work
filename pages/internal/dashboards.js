...
const DashBoards = ({ ...pageProps }) => {
  ...
  useEffect(() => {
    const getData = async () => {
      ...
      await OrdersActions.orders().then(async (response) => {
        ...
        response.data.payload.data.sort((a, b) => Date.parse(new Date(a.createdAt?.split("/").reverse().join("-"))) - Date.parse(new Date(b.createdAt?.split("/").reverse().join("-")))).map(async (ord, i) => {
          switch (ord.status) {
  ...
  if (loaded && typeof window !== 'undefined') {
    ...
    arrDates.map((date) => {
      const res = orders.filter((order) => moment(order.createdAt).format('DD/MM/YYYY') === moment(date).format('DD/MM/YYYY'))
