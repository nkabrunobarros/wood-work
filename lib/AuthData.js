...
const AuthData = async (dispatch) => {
  ...
  const getsLoggedIn = window.location.pathname !== '/' && window.location.pathname !== '/signin';

  try {
    if (getsLoggedIn) {
      const { data: { orion_permissions: orionPermissions } } = await me();
      ...
      await userPermissionsSet({
        ...
        permissions,
        newPerms: orionPermissions,
        // delete: orionPermissions.filter((perm) => perm.includes('delete')),
        // get: orionPermissions.filter((perm) => perm.includes('view')),
        // patch: orionPermissions.filter((perm) => perm.includes('change')),
        // create: orionPermissions.filter((perm) => perm.includes('add')),
      });
    }
  } catch (error) {
    console.error(error);
