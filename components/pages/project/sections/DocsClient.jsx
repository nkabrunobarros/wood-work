    ...
    try {
      await uploadFiles(data).then(async () => {
        const [foldersData, filesData] = await Promise.all([
          getFolders({ budget: order.hasBudget.object.id }),
          getFiles(order.hasBudget.object.id)
        ]);
      ...
      if (error?.response?.data.non_field_errors && error?.response?.data.non_field_errors[0] === 'A file with the same name already exists in the parent folder') toast.error('Este ficheiro já existe.');
